                ...
                match insns.[ip] with
                    ...
                    | Shl (t, (d, a, b)) ->
                        ...
                        let newval =
                            ...
                            | IntegerLong -> Store.storeLong <| (Store.loadLong v1).ShiftLeft (Store.loadInt v2 &&& 0x3F)
                        this.SetReg(d, newval)
                        next ()
                    | Shr (t, (d, a, b)) ->
                        let (v1, v2) = (this.GetReg a, this.GetReg b)
                        let newval =
                            match t with
                            | IntegerInt -> Store.storeInt <| (Store.loadInt v1 >>> (Store.loadInt v2 &&& 0x1F))
                            | IntegerLong -> Store.storeLong <| (Store.loadLong v1).ShiftRight (Store.loadInt v2 &&& 0x3F)
                        this.SetReg(d, newval)
                        next ()
                    | Ushr (t, (d, a, b)) ->
                        let (v1, v2) = (this.GetReg a, this.GetReg b)
                        let newval =
                            ...
                            | IntegerLong -> Store.storeLong <| (Store.loadLong v1).ShiftRightUnsigned (Store.loadInt v2 &&& 0x3F)
                        this.SetReg(d, newval)
                        next ()
